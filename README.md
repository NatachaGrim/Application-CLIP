...
You can name the ```📁 project_name``` and every ```📁 subfolder``` folder as you wish (with no spaces or special characters). As a consequence, the ```project_name_ontology.txt``` file must be named after ```📁 project_name```. The ```📁 _eval```, ```📁 images``` and ```📁 ontology``` folders must keep these names:
...
The ``project_name_probs.txt`` file contains a list of two probabilities that CLIP assigns to each image. This makes it possible to view the model's confidence score in assigning the two most probable classes in a keyword query. The closer the score is to 1, the higher the probability.
