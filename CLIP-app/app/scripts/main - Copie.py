    ...
    if request.method == "POST":
        ...
        if query in labels:
            ...
            if len(urls) != 0:
                ...
                urls_top_prob = ['-'] * len(name_image_top_prob)
        ...
        if not labels:
            return render_template("home.html", files=random_files, urls=urls, msg='Use a free query:', classes='', count=image_count, img_folder=img_folder)
        else:
            return render_template("home.html", files=random_files, urls=urls, msg_fr='Saisissez une requête libre ou une des étiquettes suivantes : ', msg='Use a free query OR one of these predefined classes: ', classes=', '.join(labels), count=image_count, img_folder=img_folder)
